...
void generate(int dim, char* filename, int seeds=0) {
  ...
  rank=MPI::COMM_WORLD.Get_rank();
    #endif
  //  std::cout<<"in flip_index_helper at rank="<<rank<<std::endl;
  ...
  //  if(rank==0) std::cout<<"position 1"<<std::endl;

  const double kT = 0.50;

  /*  if(rank==0){
    std::cout<<"\n in flip_index_helper"<<std::endl;
    std::cout<<ss->front_low_left_corner[0]<<"  "<<ss->front_low_left_corner[1]<<"  "<<ss->front_low_left_corner[2]<<std::endl;
    std::cout<<ss->back_up_right_corner[0]<<"  "<<ss->back_up_right_corner[1]<<"  "<<ss->back_up_right_corner[2]<<std::endl;
    }*/
  ...
  /*
    std::cout<<"range is "<<range<<"  ss->front_low_left_corner[0]+2*(rand()%((range+1)/2)) is  "<<ss->front_low_left_corner[0]+2*(rand()%((range+1)/2))<<"  ss->front_low_left_corner[0] is "<<ss->front_low_left_corner[0]<<"  2*(rand()%((range+1)/2)) is "<<2*(rand()%((range+1)/2))<< std::endl;*/
...
template <int dim> void update(MMSP::grid<dim, int>& grid, int steps) {
  ...
  //	if(rank == 0)
	  //	  std::cout<<"flip index array generated"<<std::endl;
  ...
  // if(rank == 0)
  // std::cout<<"ready for looping of color"<<std::endl;
  ...
  for (int step=0; step<steps; step++) {
    for(int sublattice=0; sublattice!= 2; sublattice++){
      ...
      /*    if(rank == 5 ){
        std::cout<<g0(grid,0)<<"  "<<g0(grid,1)<<"  "<<g0(grid,2)<<std::endl;
        std::cout<<g1(grid,0)<<"  "<<g1(grid,1)<<"  "<<g1(grid,2)<<std::endl;

      std::cout<<x0(grid,0)<<"  "<<x0(grid,1)<<"  "<<x0(grid,2)<<std::endl;
      std::cout<<x1(grid,0)<<"  "<<x1(grid,1)<<"  "<<x1(grid,2)<<std::endl;
      }*/
      ...
      for(int i=0; i!= nthreads ; i++ ) {
        ...
        //   std::cout<<"pthread_create finished at rank"<<rank<<std::endl;
      }//loop over threads 
  ...
  delete [] p_threads ; p_threads=NULL;
  delete [] mat_para ; mat_para=NULL;
